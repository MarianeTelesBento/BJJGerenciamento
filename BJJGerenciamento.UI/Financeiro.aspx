...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
            ...
            <asp:TemplateField>
                <ItemTemplate>
                    <asp:Button ID="btnDetalhesPlano"
                        ...
                        CommandArgument='<%# Eval("idPlanoAluno") %>' 
                        OnClick="btnDetalhesPlano_Click" />
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </asp:GridView>
    ...
    <script>
        ...
        function confirmarPagamento() {
            Swal.fire({
                title: 'Tem certeza?',
                text: "Você será redirecionado e o pagamento será confirmado!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    document.getElementById('<%= btnPagamentoEfetuado.ClientID %>').click();
                } else {
                    Swal.fire('Cancelado', 'O pagamento não foi efetuado.', 'error');
                }
            });
        }
    </script>
